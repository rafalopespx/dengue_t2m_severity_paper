...
for (i in 1:stacked_levels) {
  ...
  # creating plot_list
  ## 
  RR_lag<-for(j in c(5,95)){
    RR_lag_q<-RRVal_lag_list %>% 
      filter(abbrev_state == names_stacked_plot[i] & idE == j) %>% 
      arrange(lag0) %>% 
      rename(lmh = lag)
    RR_lag_q$lmh<-c("low", "mid", "high") 
    RR_lag_q<-RR_lag_q %>% 
      pivot_longer(cols = c("lmh"))
  }
  ...
  # Patchwork Plot to be saved
  plot_final[[i]]<-(plot_list[[i]]$rr_lag | plot_list[[i]]$rr_overall)+
    plot_layout(guides = "collect")
  
  # PAY ATTENTION TO THE NAME OF FILES YOU ARE SAVING, 
  # MAKE SURE IT IS THE CORRECT, TO NOT SUBSTITUTE FOR AN EXISTING ONE!
  ggsave(paste0("Outputs/Plots/gnm_", names_stacked_plot[i], "_scale_stacked_state_overall_effects.png"),
         plot = plot_final[[i]],
         width = 9,
         height = 7,
         dpi = 300)
...
ggsave(paste0("Outputs/Plots/gnm_scale_all_state_overall_effects.png"),
...
ggsave(paste0("Outputs/Plots/gnm_scale_all_state_cold_5th_effects.png"),
